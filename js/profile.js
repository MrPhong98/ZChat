(function () {
    ...
    function renderVerifiedBadge(isVerified) {
        ...
        if (preview) {
            const name = (usernameField && usernameField.value.trim()) || savedUsername || "";
            preview.innerHTML = "@" + name.toLowerCase().replace(/\s+/g, "") + getVerifiedBadgeSvg(isVerified);
        }
        const nameTitle = document.querySelector("[data-profile-name]") || document.getElementById("profileDisplayName");
        if (nameTitle) {
            const base = (usernameField && usernameField.value.trim()) || savedUsername || "";
            nameTitle.innerHTML = escapeHtmlSafe(base) + getVerifiedBadgeSvg(isVerified);
        }
    }
    ...
    icons();
    // Đồng bộ avatar + tick xanh từ tài khoản (sau khi DOM/render sẵn sàng)
    loadAvatarFromAccount();
    // Hiện tick ngay nếu local đã có (trước khi fetch xong)
